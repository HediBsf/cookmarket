...
export default function SellerFormationEditPage() {
  ...
  useEffect(() => {
    async function loadFormation() {
      try {
        const formation = await apiGet<Formation>(`/api/formations/${params.id}`);
        if (user && formation.seller?.id && formation.seller.id !== user.id) {
          setError("Vous ne pouvez pas modifier cette formation.");
          return;
        }
        setForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      await apiPatch(`/api/formations/${params.id}`, {
        ...form,
        price: Number(form.price),
      });
      setMessage("Formation modifiee avec succes.");
      window.setTimeout(() => {
      ...
      <main className="page-container">
        <div className="mx-auto max-w-3xl">
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <span className="badge">Edition de la formation</span>
              <h1 className="section-title mt-4">Modifier la formation</h1>
              <p className="mt-2 text-stone-600">Mettez à jour les informations et la photo de votre formation.</p>
            </div>
